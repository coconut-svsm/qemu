...
static uint64_t migration_page_addr = 0x8000f1c000;
...
static void write_status_register(uint8_t value) {
    ...
    uint64_t pa = migration_page_addr + STATUS_REGISTER_OFFSET;
    uint8_t buf[1] = {value};
...
static void write_data_register(uint8_t value) {
    ...
    uint64_t pa = migration_page_addr + DATA_REGISTER_OFFSET;
    uint8_t buf[1] = {value};
    cpu_physical_memory_write(pa, buf, 1);
}
...
//     uint64_t pa = migration_page_addr + DATA_BUFFER_OFFSET;
...
static void write_address_register(ram_addr_t guest_physical_addr) {
    //SnpMigrationState *s = &current_snp_migration;
    //uint64_t pa = s->migration_page_addr + DATA_BUFFER_OFFSET;
    uint64_t pa = migration_page_addr + ADDRESS_REGISTER_OFFSET;

    uint8_t buf[8];
    //uint64_t leval = cpu_to_le64(value);
...
uint8_t* snp_package_page(ram_addr_t guest_physical_addr) {
    ...
    /*
    qemu_log("KUBA: migration_page_addr: %p\n", (void*)migration_page_addr);
    uint64_t pa = migration_page_addr;// + DATA_BUFFER_OFFSET;
    return (uint8_t *) (uintptr_t) pa;
    */
    return 0;
}
