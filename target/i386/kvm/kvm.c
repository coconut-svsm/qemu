...
static int plane_from_target(uint16_t target)
{
    int ret = -EINVAL;
    int  i;
...
static int kvm_handle_plane_create_vcpu(CPUState *cpu, struct kvm_run *run)
{
    ...
    plane = plane_from_target(run->plane_event.target);
    if (plane < 0) {
        return plane;
    }
...
static int kvm_handle_snp_run_vmpl(CPUState *cpu, struct kvm_run *run)
{
       int plane = plane_from_target(run->plane_event.target);
       if (plane < 0) {
               return plane;
       }
